...
export async function createPost(data: FormData) {
  ...
  revalidatePath("/admin/posts"); revalidatePath("/"); revalidatePath("/en"); revalidatePath("/blog"); revalidatePath("/en/blog"); redirect("/admin/posts");
}
...
export async function createProject(data: FormData) {
  ...
  revalidatePath("/admin/projects"); revalidatePath("/"); revalidatePath("/en"); redirect("/admin/projects");
}
...
export async function updatePost(data: FormData) {
  ...
  await getAdminDb().collection("posts").doc(id).update({ title, slug: slugify(value(data, "slug") || title), excerpt: value(data, "excerpt"), content, translations: { en: { title: value(data, "titleEn"), excerpt: value(data, "excerptEn"), content: value(data, "contentEn") } }, category, coverImage: value(data, "coverImage"), tags: value(data, "tags").split(",").map((tag) => tag.trim()).filter(Boolean), published, publishedAt: published ? FieldValue.serverTimestamp() : null, updatedAt: FieldValue.serverTimestamp() });
  revalidatePath("/admin/posts"); revalidatePath("/"); revalidatePath("/en"); revalidatePath("/blog"); revalidatePath("/en/blog"); redirect("/admin/posts");
}
...
export async function deletePost(data: FormData) {
  ...
  await getAdminDb().collection("posts").doc(id).delete(); revalidatePath("/admin/posts"); revalidatePath("/"); revalidatePath("/en"); revalidatePath("/blog"); revalidatePath("/en/blog"); redirect("/admin/posts");
}
